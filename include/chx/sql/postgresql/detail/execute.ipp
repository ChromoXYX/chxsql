...
template <> struct visitor<tags::execute> {
...
decltype(auto) chx::sql::postgresql::connection<Stream>::async_execute(
    const std::string& prepared_statement_name,
    std::tuple<Parameters...> params, CompletionToken&& completion_token) {
    auto msg = std::tuple(
        detail::create_bind_msg(prepared_statement_name, std::move(params)),
    ...
    return net::async_combine_ng<const std::error_code&, result_sets>(
        ...
        net::detail::type_identity<
            detail::visitor<detail::tags::execute>::operation<Stream>>{},
        std::tuple(this), std::false_type{}, std::move(msg));
}
